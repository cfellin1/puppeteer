...
class FrameManager extends EventEmitter {
  ...
  constructor(client, frameTree, page) {
    ...
    this._client.on('Page.lifecycleEvent', event => {
      console.log('passing on page lifecycle event:', event);
      this.emit(FrameManager.Events.LifecycleEvent);
    });
    this._eventListeners = [
      helper.addEventListener(this._client, Connection.Events.LifecycleEvent, () => {
        console.log('FrameManager emitting event');
        this.emit(Connection.Events.LifecycleEvent);
      })
    ];
